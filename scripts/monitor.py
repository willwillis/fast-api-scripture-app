...
class ScriptureMonitor:
    ...
    def run_full_test_suite(self, warm_up: bool = True) -> Dict[str, Any]:
        ...
        for endpoint, name in endpoints:
            ...
            results["tests"][name.lower().replace(" ", "_")] = self.test_endpoint(
                endpoint, name
            )
    ...
    def send_to_grafana(
        self, results: Dict[str, Any], url: str, username: str, api_key: str
    ):
        ...
        try:
            ...
            for test_name, result in results["tests"].items():
                if result["status"] == "success":
                    ...
                    metric_name = (
                        f'scripture_app_response_time_seconds{{endpoint="{test_name}",'
                        f'instance="local"}}'
                    )
                    metrics_lines.append(f"{metric_name} {response_time}")
                    metrics_lines.append(
                        f'scripture_app_test_success{{test="{test_name}",'
                        f'instance="local"}} 1'
                    )
                else:
                    metrics_lines.append(
            ...
            if health_result.get("status") == "success":
                metrics_lines.append('scripture_app_health_status{instance="local"} 1')
            else:
                metrics_lines.append('scripture_app_health_status{instance="local"} 0')
            ...
            if response.status_code == 200:
                ...
                self.log(
                    f"Failed to send metrics: {response.status_code} - "
                    f"{response.text}",
                    "ERROR",
                )
...
def main():
    ...
    try:
        ...
        if args.grafana_url and args.grafana_username and args.grafana_api_key:
            monitor.send_to_grafana(
                results, args.grafana_url, args.grafana_username, args.grafana_api_key
            )
